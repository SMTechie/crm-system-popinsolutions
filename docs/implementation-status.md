...
For Vercel, point the project at the repository root, keep the `apps/web` Next.js application as the build target, and configure `NEXT_PUBLIC_API_URL` to the deployed API. The API is intended to run as its Docker image or another managed Node service; persistent uploads must use `STORAGE_PROVIDER=s3` (or `r2`) and the storage bucket/key/secret/endpoint variables in production. `STORAGE_PUBLIC_BASE_URL` is optional when the bucket is publicly readable; otherwise the API should be extended with authenticated download URLs before exposing private attachments.
